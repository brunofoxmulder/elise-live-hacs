...
def active_pipeline_context(
    ...
    """Resolve conversation, device and original Context of one active STT run.
    ...
    try:
        ...
        candidates: list[tuple[str, str | None, Context | None]] = []
        ...
        if len(candidates) == 1:
            return candidates[0]
        # No temporal guess: simultaneous runs get independent anonymous turns.
        # This also prevents accidentally sharing the other satellite's audio.
        if len(candidates) > 1:
            _LOGGER.warning("Ambiguous active STT pipelines; provenance unavailable")
    except Exception:  # noqa: BLE001
        _LOGGER.debug(
