...
class LiveModelConversationAgent(conversation.ConversationEntity):
    ...
    def _fire_conversation_entry(
        ...
        _LOGGER.debug(
            "Fired elise_live_conversation_entry: user=%r assistant=%r",
    ...
    async def _async_get_llm_api(
        ...
        try:
            llm_api = await async_load_tools(self.hass, config, llm_context)
            api_prompt = llm_api.api_prompt if llm_api else ""
            ...
            live_tools = _add_end_conversation_tool(
                _format_tools_for_live(
                    llm_api.tools if llm_api else [],
                    llm_api.custom_serializer if llm_api else None,
                    encourage_web_search,
                )
            )
            if not transcribe_output and show_text:
...
class GeminiLiveConversationAgent(LiveModelConversationAgent):
    ...
    integration_name = "Élise Live — Gemini"
    conversation_event = "elise_live_conversation_entry"
...
class GPTRealtimeConversationAgent(LiveModelConversationAgent):
    ...
    integration_name = "Élise Live — OpenAI"
    conversation_event = "elise_live_openai_conversation_entry"
