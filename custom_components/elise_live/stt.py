...
class LiveModelSTT(SpeechToTextEntity):
        ...
        try:
            llm_api = await async_load_tools(
                ...
                llm_context=llm.LLMContext(
                    ...
                    context=pipeline_context if pipeline_context is not None else Context(),
                    language=metadata.language or "en",
                    assistant="conversation",
                    device_id=device_id,
                ),
            )
            ha_tools = llm_api.tools if llm_api else []
    ...
    async def _async_process_audio_stream_sdk(
        ...
        # Capture provenance before the audio task is detached from the pipeline.
